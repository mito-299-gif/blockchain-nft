...
<body>
  <div class="wrap">
    ...
    <!-- Header -->
    <header>
      <div class="logo"><span class="badge-icon">🎓</span>Learn<span class="accent">Badge</span></div>
      <button id="walletBtn" onclick="connectWallet()">🦊 Connect Wallet</button>
    </header>

    <!-- Step 1: Hero — กด Connect ก่อน -->
    <div id="heroSection">
      <div class="hero">
    ...
    <!-- Step 2: เลือก Network หลัง Connect แล้ว -->
    <div id="networkSection" style="display:none;">
      <div class="hero" style="padding-top:3rem;">
        <h1 style="font-size:clamp(1.8rem,4vw,3rem);margin-bottom:0.5rem;">เลือก <span class="grad">Network</span></h1>
        <p style="margin-bottom:2rem;">// wallet เชื่อมแล้ว กรอก Chain ID เพื่อเข้าใช้งาน</p>

        <div
          style="background:#111827;border:1px solid #1f2937;border-radius:16px;padding:1.8rem 2rem;max-width:420px;margin:0 auto 2rem;text-align:left;">

          <!-- Wallet Info -->
          <div
            style="display:flex;align-items:center;gap:10px;background:#0d1117;border:1px solid #1f2937;border-radius:10px;padding:10px 14px;margin-bottom:1.4rem;">
            <div style="width:10px;height:10px;background:#34d399;border-radius:50%;flex-shrink:0;"></div>
            <div style="font-family:'Space Mono',monospace;font-size:0.78rem;color:#34d399;" id="connectedAddr">—</div>
          </div>

          <!-- Chain ID Input -->
          <div style="margin-bottom:1rem;">
            <label
              style="display:block;font-size:0.75rem;color:#64748b;font-family:'Space Mono',monospace;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:6px;">Chain
              ID</label>
            <input id="inputChainId" type="text" placeholder="เช่น 11155111" oninput="previewChainId(this.value)"
              style="width:100%;background:#0d1117;border:1px solid #1f2937;border-radius:8px;padding:11px 14px;color:#f1f5f9;font-family:'Space Mono',monospace;font-size:0.85rem;outline:none;"
              onfocus="this.style.borderColor='#6366f1'" onblur="this.style.borderColor='#1f2937'" />
            <div id="chainIdPreview"
              style="font-size:0.7rem;color:#34d399;font-family:'Space Mono',monospace;margin-top:5px;min-height:18px;">
            </div>
          </div>

          <!-- ปุ่มดึงจาก MetaMask -->
          <button onclick="detectFromMetaMask()"
            style="width:100%;background:rgba(245,158,11,0.12);border:1px solid rgba(245,158,11,0.35);color:#f59e0b;padding:10px;border-radius:8px;font-size:0.8rem;font-family:'Space Mono',monospace;cursor:pointer;margin-bottom:1rem;transition:all 0.2s;"
            onmouseover="this.style.background='rgba(245,158,11,0.2)'"
            onmouseout="this.style.background='rgba(245,158,11,0.12)'">
            🦊 ดึง Chain ID จาก MetaMask อัตโนมัติ
          </button>

          <div id="detectResult"
            style="font-size:0.72rem;font-family:'Space Mono',monospace;min-height:18px;margin-bottom:1rem;"></div>
        </div>
    ...
    <div id="appSection" style="display:none;">
      <div style="padding:2rem 2rem 0;max-width:1100px;margin:0 auto;">
        <div class="tabs">
          ...
          <button class="tab" onclick="switchTab('mint')" id="mintTab" style="display:none;">⚡ ออก Badge</button>
        </div>
      </div>

      <div id="tabMy" class="section">
        <div class="section-title">Badge ของฉัน</div>
        <div class="section-sub" id="networkLabel">// NFT certificates on LabChain</div>
        <div class="stats-bar">
          <div class="stat-card">
          ...
          <div class="stat-card">
            <div class="num" style="color:var(--gold)" id="networkName">—</div>
        ...
        <div class="mint-form">
          <div style="margin-bottom:1.4rem;">
            ...
            <div class="badge-presets">
              <button class="preset-btn" onclick="fillPreset('Blockchain Beginner','ผ่านการทดสอบพื้นฐาน Blockchain')">🔗
                Blockchain Beginner</button>
              <button class="preset-btn" onclick="fillPreset('Smart Contract Dev','เขียน Smart Contract สำเร็จ')">⚡
                Smart Contract</button>
              <button class="preset-btn" onclick="fillPreset('Web3 Explorer','สำรวจ Web3 ครบทุกด้าน')">🌐 Web3
                Explorer</button>
              <button class="preset-btn" onclick="fillPreset('Hackathon Winner','ชนะการแข่งขัน Blockchain 2026')">🏆
                Hackathon Winner</button>
            </div>
          </div>
          <div class="form-group"><label>Student Wallet Address</label><input type="text" id="mintTo"
              placeholder="0x..." /></div>
          <div class="form-group"><label>Badge Name</label><input type="text" id="mintName"
              placeholder="เช่น Blockchain Beginner" /></div>
          <div class="form-group"><label>Activity / คำอธิบาย</label><textarea id="mintActivity"
              placeholder="เช่น ผ่านการทดสอบพื้นฐาน Blockchain 101"></textarea></div>
          <div class="form-group"><label>Image/Metadata URI</label><input type="text" id="mintURI"
              placeholder="ipfs://... หรือ https://..." /></div>
          <button class="submit-btn" id="mintBtn" onclick="mintBadge()">⚡ Mint Badge</button>
          <div id="mintStatus"
            style="margin-top:1rem;text-align:center;font-family:'Space Mono',monospace;font-size:0.85rem;display:none;">
          </div>
        </div>
      </div>
    </div>
    ...
    <script>
      // ============================================================
      //  CONFIG — ล็อก CONTRACT_ADDRESS ไว้ในโค้ด
      // ============================================================
      const CONTRACT_ADDRESS = '0xE76CAF6C344230f319D70f97F712a424E6b61B72';
      let CHAIN_ID = '0xaa36a7'; // อัปเดตจาก UI
      ...
      async function connectWallet() {
        ...
        try {
          ...
          // แสดง address ใน header และ network section
          document.getElementById('walletBtn').textContent = shortenAddr(account);
          document.getElementById('walletBtn').classList.add('connected');
          document.getElementById('connectedAddr').textContent = account;

          // ไปหน้า Step 2 — เลือก Network
          document.getElementById('heroSection').style.display = 'none';
          document.getElementById('networkSection').style.display = 'block';

          // ดึง Chain ID จาก MetaMask ให้อัตโนมัติเลย
          await detectFromMetaMask();
      ...
      // ── Step 2: กด "เข้าใช้งาน" ──────────────────────────────
      async function enterApp() {
        const chainRaw = document.getElementById('inputChainId').value.trim();
        if (!chainRaw) { toast('❌ กรุณาใส่ Chain ID ก่อน', 'error'); return; }

        const chainNum = parseInt(chainRaw);
        CHAIN_ID = '0x' + (isNaN(chainNum) ? parseInt(chainRaw, 16) : chainNum).toString(16);
        ...
        try {
          ...
          // อัปเดต network label
          const netName = NETWORK_NAMES[chainNum] || `Chain ${chainNum}`;
          document.getElementById('networkName').textContent = netName;
          document.getElementById('networkLabel').textContent = `// NFT certificates on ${netName}`;

          // ซ่อน Step 2 → แสดง Step 3
      ...
      async function detectFromMetaMask() {
        ...
        if (!window.ethereum) { resultEl.style.color = '#f43f5e'; resultEl.textContent = '❌ MetaMask ไม่พบ'; return; }
        try {
          resultEl.style.color = '#64748b'; resultEl.textContent = '⏳ กำลังดึง...';
          ...
          const netName = NETWORK_NAMES[chainDec] || `Chain ID: ${chainDec}`;
          resultEl.style.color = '#34d399';
          resultEl.textContent = `✅ ${netName} (${chainDec})`;
        } catch (e) {
        ...
        try {
          ...
          for (const tid of tokenIds) {
            try {
              ...
              myBadges.push({ tokenId: Number(tid), badgeName: b.badgeName, activity: b.activity, issuedAt: Number(b.issuedAt) });
            } catch (e) {
              myBadges.push({ tokenId: Number(tid), badgeName: `Badge #${tid}`, activity: '', issuedAt: 0 });
            }
          }
          showStatus(''); renderBadges(myBadges);
        } catch (e) {
      ...
      async function mintBadge() {
        ...
        try {
          const cws = new ethers.Contract(CONTRACT_ADDRESS, ABI, signer);
          const tx = await cws.mintBadge(to, name, activity, uri);
      ...
      function openModal(b) {
        ...
        document.getElementById('modalMeta').innerHTML =
          `Token ID: <span>#${b.tokenId}</span><br>ออกเมื่อ: <span>${date}</span><br>Contract: <span>${CONTRACT_ADDRESS.slice(0, 10)}...</span>`;
      }
      ...
      function showStatus(type, msg = '') {
        ...
        if (type === 'loading') { el.style.display = 'block'; el.className = ''; el.innerHTML = '<div class="spinner"></div><p>กำลังโหลด...</p>'; }
        else if (type === 'empty') { el.style.display = 'block'; el.className = ''; el.innerHTML = '<div class="empty"><div class="icon">🎓</div><p>ยังไม่มี Badge</p></div>'; }
        else if (type === 'error') { el.style.display = 'block'; el.className = 'error'; el.textContent = msg; el.style.whiteSpace = 'pre-line'; }
        else { el.style.display = 'none'; el.textContent = ''; }
      }
      function setMintStatus(type, msg = '') {
        const el = document.getElementById('mintStatus');
        el.style.display = type ? 'block' : 'none';
        if (type === 'loading') { el.innerHTML = '<div class="spinner" style="width:26px;height:26px;border-width:2px;margin:0 auto 6px"></div>' + (msg || 'กำลัง Mint...'); el.style.color = 'var(--muted)'; }
        else if (type === 'success') { el.textContent = msg; el.style.color = 'var(--accent2)'; el.style.whiteSpace = 'pre-line'; }
